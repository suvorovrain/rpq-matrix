...
namespace rpq
{
    ...
    private:
        ...
        std::vector<matrix> mcsr_matrices;
        std::vector<matrix> mcsc_matrices;
                       ...
                       bool skip_closure = false)
        {
            ...
            case CONC:
            {
                ...
                else
                {
                    ...
                    matrix tmp;
                    while (rl.size() > 2)
                    { // Check size if there are more than three elements
                        it1 = it2 = rl.begin();
                        ++it2;
                        ...
                        if (it1_min->is_tmp)
                            wrapper::destroy(it1_min->m);
                        if (it2_min->is_tmp)
                            wrapper::destroy(it2_min->m);
            ...
            case QUESTION:
            {
                ...
                matrix Id = wrapper::id(std::max(height, width));
                                 ...
                                 bool skip_closure = false)
        {
            ...
            {
                list_type ll, rl;
                if (node->e1->type == STAR || node->e1->type == PLUS)
                { // Special case!
                    ...
                    {                           // product of the right part
                        ...
                        if (it1->is_tmp)
                            wrapper::destroy(it1->m);
                        if (it2->is_tmp)
                            wrapper::destroy(it2->m);
                        while (++it2 != rl.rend())
                        {
                            matrix A;
                            s_matrix sA;
    ...
    public: // GB!
                        ...
                        const uint n_preds, const uint n_triples)
        {
            ...
            mcsr_matrices.resize(n_preds + 1);
            mcsc_matrices.resize(n_preds + 1);
            uint64_t space = 0;
            uint i;
            FILE *f;
